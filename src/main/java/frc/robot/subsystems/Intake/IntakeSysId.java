...
public class IntakeSysId extends Intake {
  ...
  public IntakeSysId(IntakeIO io, PivotIO pivotIO) {
    super(io, pivotIO);
    ...
    // ? The hopper roller and the intake roller are both the same, so the hopper roller is used
    // here.
    routine =
        new SysIdRoutine(
            ...
            new SysIdRoutine.Mechanism(
                ...
                (log) -> {
                  log.motor("IntakeMotor")
                      ...
                      .linearPosition(
                          intakePosition.mut_replace(
                              io.getLinearPosition().to(PARTsUnitType.Inch), Inches))
                      .linearVelocity(
                          intakeVelocity.mut_replace(
                              (super.getIntakeRPM()
                                      * Math.PI
