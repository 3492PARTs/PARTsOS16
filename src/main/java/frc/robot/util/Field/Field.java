...
public interface Field {
  ...
  public static Pose3d transformToOppositeAlliance(Pose3d pose) {
    ...
    return new Pose3d(
        rotated.getTranslation().plus(new Translation3d(LENGTH, WIDTH, 0)), rotated.getRotation());
  }
  ...
  enum NamedTags {
    ...
    public Pose3d getLocation() {
      return Robot.isBlue() ? tag.getLocation() : transformToOppositeAlliance(tag.getLocation());
    }
  ...
  AprilTag APRILTAGS[] = {
    ...
    new AprilTag(
        ...
        new Pose3d(
            ...
            new Rotation3d(
                Units.degreesToRadians(30), Units.degreesToRadians(0), Units.degreesToRadians(0)))),
    new AprilTag(
        5,
        new Pose3d(
            new Translation3d(
    ...
    new AprilTag(
        ...
        new Pose3d(
            new Translation3d(
                ...
                Units.inchesToMeters(12.13)),
            new Rotation3d(
                Units.degreesToRadians(0), Units.degreesToRadians(0), Units.degreesToRadians(60)))),
    new AprilTag(
        9,
        new Pose3d(
            new Translation3d(
    ...
    new AprilTag(
        ...
        new Pose3d(
            new Translation3d(
                ...
                Units.inchesToMeters(58.50)),
            new Rotation3d(
                Units.degreesToRadians(0), Units.degreesToRadians(0), Units.degreesToRadians(54)))),
    new AprilTag(
        13,
        new Pose3d(
            new Translation3d(
    ...
    new AprilTag(
        ...
        new Pose3d(
            new Translation3d(
                Units.inchesToMeters(144.0 /*-/13.5*/),
                Units.inchesToMeters(158.50),
                Units.inchesToMeters(12.13)),
            new Rotation3d(
                Units.degreesToRadians(0),
  ...
  public static boolean seesReefTags(int[] reefArray, int id) {
    for (int element : reefArray) {
      if (element == id) {
        return true;
      }
    }
  ...
  /*** REEF ***/
  Translation2d ALLIANCE_REEF_CENTER =
      new Translation2d(Units.inchesToMeters(144.0 + (93.5 - 14.0 * 2) / 2), Field.WIDTH / 2);
  ...
  public enum CoralStation {
    BLUE_CD_CORAL_STATION(
        NamedTags.BLUE_CD_CORAL_STATION,
        new Translation2d(0, Field.WIDTH / 2 - Units.inchesToMeters(109.13)),
        new Translation2d(Units.inchesToMeters(65.84), 0)),
    BLUE_KL_CORAL_STATION(
        NamedTags.BLUE_KL_CORAL_STATION,
        new Translation2d(0, Field.WIDTH / 2 + Units.inchesToMeters(109.13)),
        new Translation2d(Units.inchesToMeters(65.84), Field.WIDTH)),
    RED_CD_CORAL_STATION(
        NamedTags.RED_CD_CORAL_STATION,
        new Translation2d(Field.LENGTH, Field.WIDTH / 2 + Units.inchesToMeters(109.13)),
        new Translation2d(Field.LENGTH - Units.inchesToMeters(65.84), Field.WIDTH)),
    RED_KL_CORAL_STATION(
        NamedTags.RED_KL_CORAL_STATION,
        new Translation2d(Field.LENGTH, Field.WIDTH / 2 - Units.inchesToMeters(109.13)),
        new Translation2d(Field.LENGTH - Units.inchesToMeters(65.84), 0));
    ...
    private CoralStation(
        NamedTags correspondingAprilTag,
        Translation2d blueOriginLineStart,
        Translation2d blueOriginLineEnd) {
    ...
    // Kalimul said to add a comment that this is getting the closest point to a
    // line
    /*
     ...
     *
     * Vector2D lineStartToEnd = new Vector2D(lineEnd.getX() - lineStart.getX(),
     * lineEnd.getY() - lineStart.getY());
     * Vector2D lineStartToPoint = new Vector2D(robot.getX() - lineStart.getX(),
     * robot.getY() - lineStart.getY());
     *
     ...
     * double percentToIgnoreFromDriverStationSide =
     * (Constants.WIDTH_WITH_BUMPERS_METERS / 2 + (isCDCoralStation() ?
     * Settings.Clearances.CLEARANCE_DISTANCE_CORAL_STATION_ALIGN_FUNNEL_SIDE :
     * Settings.Clearances.CLEARANCE_DISTANCE_CORAL_STATION_ALIGN_FROGGY_SIDE)) /
     * coralStationLength;
     * double percentToIgnoreFromSideWallSide = (Constants.WIDTH_WITH_BUMPERS_METERS
     * / 2 + (isCDCoralStation() ?
     * Settings.Clearances.CLEARANCE_DISTANCE_CORAL_STATION_ALIGN_FROGGY_SIDE :
     * Settings.Clearances.CLEARANCE_DISTANCE_CORAL_STATION_ALIGN_FUNNEL_SIDE)) /
     * coralStationLength;
     *
     * t = Math.max(percentToIgnoreFromDriverStationSide, Math.min(1 -
     * percentToIgnoreFromSideWallSide, t));
     *
     * Translation2d closestPointOnCoralStation = new Translation2d(lineStart.getX()
     * + t * lineStartToEnd.x, lineStart.getY() + t * lineStartToEnd.y);
     *
     * return new Pose2d(closestPointOnCoralStation,
     * correspondingAprilTag.getLocation().toPose2d().getRotation()).transformBy(new
     * Transform2d(
     * Constants.LENGTH_WITH_BUMPERS_METERS / 2 +
     * Settings.Swerve.Alignment.Targets.TARGET_DISTANCE_FROM_CORAL_STATION,
     * 0, Rotation2d.kZero));
     * }
     */
     ...
     * Constants.LENGTH_WITH_BUMPERS_METERS / 2 +
     * Settings.Swerve.Alignment.Targets.TARGET_DISTANCE_FROM_CORAL_STATION,
     * direction * Settings.Swerve.Alignment.Targets.
     * TARGET_DISTANCE_FROM_CORAL_STATION_LEFT_RIGHT,
     * Rotation2d.kZero));
     * }
     *
     ...
     * Vector2D C = new
     * Vector2D(CommandSwerveDrivetrain.getInstance().getPose().getTranslation());
     *
     ...
     * }
     * else {
     * return RED_CD_CORAL_STATION.getDistanceToStation() <
     * RED_KL_CORAL_STATION.getDistanceToStation()
     * ? RED_CD_CORAL_STATION
     * : RED_KL_CORAL_STATION;
     * }
     * }
