...
public class SOTMCalculator {
   ...
   * <p>Value = RPM
   *
   * <p>Make sure to use the gotten distance from {@link frc.robot.util.Trench#getDistance(Pose2d,
   * Pose2d) Trench.getDistance()} to properly calculate the right RPM.
   */
  ...
  /**
   * Creates and populates the {@link #rpmTable RPM Table} with the correct position-to-rpm values
   * based off of the current zone code.
   *
   * <p>This function should automatically get called at runtime, so there's no need to expose this
   * function.
   */
  private static InterpolatingDoubleTreeMap initRpmTable() {
    ...
    table.put(
        new PARTsUnit(7.6, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
    ...
    table.put(
        ...
        2525.0);
    table.put(
        new PARTsUnit(9.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        2550.0);
    table.put(
        new PARTsUnit(12.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        2925.0);
    table.put(
        new PARTsUnit(15.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        3125.0);
    table.put(
        new PARTsUnit(18.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        3350.0);
    table.put(
        new PARTsUnit(21.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        3675.0);
    table.put(
        new PARTsUnit(24.4375, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        3875.0);
    table.put(
        new PARTsUnit(26.8125, PARTsUnitType.Foot).to(PARTsUnitType.Meter)
            - TurretConstants.TURRET_OFFSET_CENTER.to(PARTsUnitType.Meter),
        4175.0);
  ...
  /**
   * Gets the RPM from the distance to the goal pose using the {@link #rpmTable RPM Table}.
   *
   * @param robotPose The current pose of the robot, used to calculate the distance to the goal.
   ...
   *
   * @param robotPose The current pose of the robot, used to calculate the distance to the hub.
  ...
  /**
   * Calculates the target robot pose based off the pose, velocity, and the calculated {@link
   * #getFlightTimeToGoal(Pose2d, Pose2d) Time of Flight}.
   *
  ...
  public static Pose2d getTargetPose(Pose2d robotPose, Transform2d robotVelocity, double tof) {
    ...
    Transform2d tofPose =
        new Transform2d(robotVelocity.getX() * tof, robotVelocity.getY() * tof, new Rotation2d());
